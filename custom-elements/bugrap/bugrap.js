Polymer({
    ...
    events: function events() {
        ...
        $(this.$.report_edit_container).on( "resize", function(event, ui) {
            self.setReportCommentHeight();
            self.editPanelResized = true;
        });

        //Update site when project is changed
        self.$.project_select.addEventListener('iron-select', function(){
            self.projectSelect(this.selectedItem.value);
        });

        //Update grid when version changed
        self.$.version_menu.addEventListener('iron-select', function(){
            self.updateDistributionBarValues();
            self.updateReportGrid();
        });

        self.$.search_reports.addEventListener('change', function(){
            self.updateReportGrid();
        });

        $(document).on('click', function(event) {
            var statusDialog = $('.status-dialog');
            if(!$(event.target).closest('.status-dialog').length && !$(event.target).closest('#status_select').length) {
                if(statusDialog.is(":visible")) {
                    statusDialog.addClass('closed');
                    $(self.$.status_select_down).show();
                    $(self.$.status_select_up).hide();
                }
            }
        });

        $(self.$.status_select).on('click', self.toggleStatusSelect);
    ...
    setEditResizeable: function setEditResizeable() {
        var $reportEditContainer = $(this.$.report_edit_container);
        ...
        this.editPanelResized = false;
        $(this.$.handle).removeClass('visible-hidden');
